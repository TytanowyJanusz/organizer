...
angular
  ...
  .controller('MwlDynamicDirectiveTemplateCtrl', ["$compile", "$scope", "$attrs", "$element", "$templateCache", "$log", "calendarConfig", function($compile, $scope, $attrs, $element, $templateCache, $log, calendarConfig) {
    ...
    $scope.$watchGroup([$attrs.overrides, 'vm.templatesLoaded'], function(values) {

    // EO2
      var overrides = values[0];
...
angular
  ...
  .factory('calendarEventTitle', ["calendarDateFilter", "calendarTruncateEventTitleFilter", function(calendarDateFilter, calendarTruncateEventTitleFilter) {
    ...
    function yearView(event) {
      return event.title + ' (' + calendarDateFilter(event.startsAt, 'datetime', true) + ')<br>';
    }

    function monthView(event) {
      return event.title + ' (' + calendarDateFilter(event.startsAt, 'time', true) + ')<br>';
    }
    ...
    function weekViewTooltip(event) {
      return event.title + (event.orgReservedTerm.comments ? '- komentarz: ' + event.orgReservedTerm.comments : '');
    }

    function dayView(event) {
      return event.allDay ? event.title : calendarTruncateEventTitleFilter(event.title+'<br>', 20, event.height);
    }
