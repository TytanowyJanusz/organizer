      ...
      function TooltipFactory(element, config) {
        ...
        $tooltip.init = function() {
          ...
          if (options.container === 'self') {
             ...
             * EO2 FIX
             */
            tipContainer = tipContainer.last();
            /*
             * EO2 FIX END
             */
          }
        ...
        $tooltip.$applyPlacement = function() {
          ...
          if (autoPlace) {
             ...
             * EO2 FIX
             */
            if (/bottom/.test(originalPlacement) && elementPosition.bottom + tipHeight + 33 > viewportPosition.height) {
                    /*
                     * +33 for 'TODAY' row
                     * EO2 FIX END
                     */
              placement = originalPlacement.replace('bottom', 'top');
            } else if (/top/.test(originalPlacement) && elementPosition.top - tipHeight + 33 < viewportPosition.top) {
...
//          } else {
            ...
            if (delta.top !== 0) {
                // picker must be moved
                if (rightEdgeOffset + actualWidth + 5 < viewportDimensions.width) 
                    // enough space on right
                    delta.left = position.left - elementOffset.left + actualWidth + 5;
                else if (leftEdgeOffset - actualWidth - 5 >0) 
  ...
  angular.module('mgcrea.ngStrap.datepicker', [ 'mgcrea.ngStrap.helpers.dateParser', 'mgcrea.ngStrap.helpers.dateFormatter', 'mgcrea.ngStrap.tooltip' ]).provider('$datepicker', function() {
    ...
    return {
      ...
      link: function postLink(scope, element, attr, controller) {
        ...
        controller.$parsers.unshift(function(viewValue) {
          var date;
          // EO2 FIX: RESET STATES
          validateAgainstMinMaxDate(null);
          if (!viewValue) {
            //controller.$setValidity('date', true);
            return null;
          }
